...
def get_model_folder(
    log_dir: Union[Path, str], project_name: Union[Path, str], experiment_name: Optional[str] = ""
):  # noqa E501
    ...
    if experiment_name == "":
        subdirectories = [
            d for d in model_log.iterdir() if d.is_dir() and d.name.startswith("version_")
        ]
        sorted_subdirectories = sorted(
            subdirectories, key=lambda d: int(d.name.split("_")[1]), reverse=True
        )  # noqa E501
        model_folder = sorted_subdirectories[0].name
        model_folder = Path(log_dir, project_name, model_folder)  # type: ignore
    else:
        model_folder = Path(log_dir, project_name, experiment_name)  # type: ignore
    return model_folder
...
def write_loss_plot(
    file: Path, destination_file: Path, loss: str = "val_loss"
) -> None:  # noqa E501
...
if __name__ == "__main__":
    ...
    write_loss_plot(Path(folder, "metrics.csv"), Path(project_dir, "assets", "validation_loss.jpg"))
    model = get_model(folder)
    compiled_model = torch_geometric.compile(model)
    torch.save(model, "default_twin_predictor_model.pt")
